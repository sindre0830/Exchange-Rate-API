...
The REST service, https://restcountries.eu/, allows for alpha code filtering which would be beneficial when getting all the bordering countries' currencies but doesn't seem to allow for field filtering as well. This means that I have to decide on either do one request per bordering country or do one request and store a massive amount of data for each bordering country. I decided on the latter since requests are expensive for the REST service, but if it was wrong you can check out my former solution [here](https://git.gvk.idi.ntnu.no/course/prog2005/prog2005-2021-workspace/sindre0830/assignment-1/-/blob/9d4bf54371ce811aae325cad24a74dc5c549d641/api/countryCurrency.go).
